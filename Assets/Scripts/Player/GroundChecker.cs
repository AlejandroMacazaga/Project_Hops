...
namespace Player
{
    public class GroundChecker : ValidatedMonoBehaviour {
        ...
        void Awake()
        {
            ...
            if (_controller == null) { Debug.LogError("GroundChecker did not find a CharacterController component."); }
        }
        ...
        void Update()
        {
            ...
            if (_controller && _controller.isGrounded) {
                CheckGround(new Vector3(transform.position.x, transform.position.y - (_controller.height / 2) + startDistanceFromBottom, transform.position.z));
            }
        }
        ...
        void OnDrawGizmosSelected()
        {
            if (showDebug)
            {
                ...
                Vector3 startPoint = new Vector3(transform.position.x, transform.position.y - (_controller.height / 2) + startDistanceFromBottom, transform.position.z);
                Vector3 endPoint = new Vector3(transform.position.x, transform.position.y - (_controller.height / 2) + startDistanceFromBottom - sphereCastDistance, transform.position.z);
